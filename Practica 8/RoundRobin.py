...
class RR:
    def __init__(self, root):
        self.root = root
        self.root.title("Simulación Round Robin")
    ...
    #! Función nueva para el botón de Reanudar
    ...
    def run_process(self):
        if self.current_process:
            ...
            if self.current_process.progress < self.current_process.expected_time:
                #! Modifiqué las condicionales para que no haya problemas y se cambie el estado de un proceso que ya ha terminado.
                ...
                if self.current_process.pause_flag:
                    #! Esto lo comenté y mejor lo agregué a la función del botón Reanudar
                    # self.processes[self.process_index - 1].pause_flag = False
                    self.processes[self.process_index - 1].state = "Pausado"
                    ...
                    if self.process_index == len(self.processes):
                        ...
                        self.start_simulation()
                #! Agregué esto, esto es basicamente el quantum
                elif self.quantum >= self.processes[self.process_index - 1].quantum:
                    self.root.after_cancel(self.current_process.after_id)
                    #! El estado cambia a Ejecucion
                    if self.current_process.previous_state == "Pausado":
                        self.current_process.previous_state = "-"
                        self.current_process.state = "Listo"
                        self.current_process.progress = self.current_process.progress
                        self.update_canvas()
                        self.current_process = None
                        if self.process_index == len(self.processes):
                            self.restart_process()
    ...
    def kill_process(self):
        ...
        self.quantum = 0
        self.root.after_cancel(self.current_process.after_id)
        #! Se hizo esto para que cuando presionaramos el botón de Matar en el primer proceso no matara al elemento final.
    ...
    def update_canvas(self):
        ...
        for i, process in enumerate(self.processes):
            ...
            if process.state == "Listo":
                ...
                self.canvas.create_text(x1, y1 - 10, anchor="w", text=process.name+" - Pausado")
...
class Process:
    def __init__(self):
        ...
        self.expected_time = self.expected_time = random.randint(4,8)
        self.start_time = -1
        self.finish_time = -1
        self.total_time = 0
        self.state = "Listo"
