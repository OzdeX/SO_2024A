...
class SRT:
    def __init__(self, root):
        self.root = root
        self.root.title("Simulación Shortest Remaining Time")
        ...
        self.quantum_label = ttk.Label(root,text="Quantum: 3",font=("Arial",12))
        self.quantum_label.place(x=650,y=400)
    ...
    def check_possible_minor_time(self,process):
        last_index = len(self.processes) - 1

        # if process.expected_time > self.processes[last_index].expected_time:
        if process.expected_time - process.progress > self.processes[last_index].expected_time:
            #print(process.ID,":",process.expected_time,"-",self.processes[last_index].ID,":",self.processes[last_index].expected_time)
            if self.processes[last_index].new_process:
                self.processes[last_index].new_process = False
                return True
            else:
                return False
        else:
        ...
        if self.count_ready_processes() > 0:
            ...
            if index == None:
                ...
                for p in self.processes:
                    if p.state not in ["Terminado","Muerto"]:
                        p.previous_quantum = None
                self.update_canvas()
    ...
    def restart_process_Btn(self):
        ...
        if index == None:
            ...
            self.current_process.previous_quantum = None
            self.update_canvas()
            self.run_process()
        ...
        if not self.current_process:    
            if self.process_index <= len(self.processes):
                ...
                for i, p in enumerate(self.processes):
                    if p.expected_time < min_time and (not p.previous_quantum and p.state not in ["Terminado","Muerto", "Pausado"]):
    ...
    def run_process(self):
        ...
        if self.current_process:
            ...
            if self.current_process.progress < self.current_process.expected_time:
                ...
                if self.check_possible_minor_time(self.current_process) and self.processes[-1].state not in ["Terminado","Muerto", "Pausado"]:
                    self.current_process.state = "Listo"
                    self.current_process.final_process = False
                    self.current_process.previous_quantum = True
                    self.current_process = self.processes[len(self.processes) - 1]
                    self.current_process.progress = 0
                    self.quantum = 0
                    self.update_canvas()
                ...
                elif self.quantum >= self.current_process.quantum:
                    self.root.after_cancel(self.current_process.after_id)
                    if self.current_process.previous_state == "Pausado":
                        self.current_process.previous_state = "-"
                        self.current_process.state = "Listo"
                        self.current_process.previous_quantum = True
                        self.update_canvas()
                        self.current_process = None
    ...
    def update_canvas(self):
        ...
        for i, process in enumerate(self.processes):
            ...
            if process.state == "Listo":
                ...
                self.canvas.create_text(x1, y1 - 10, anchor="w", text=process.name + "    Tiempo esperado: "+ str(process.expected_time))
                ...
                self.canvas.create_text(x1, y1 - 10, anchor="w", text=process.name+" - Pausado" + "    Tiempo esperado: "+ str(process.expected_time))
